...
describe('Genesis', () => {
	it(`should return ${Paths.CMT_PATH_HOME}`, async () => {
		expect(getHomePath(1)).toStrictEqual(Paths.CMT_PATH_HOME)
	})
	...
	it('should generate `ed25519` configuration files with `distributeInitialSupply=true`', async () => {
		...
		file.remove(Paths.CMT_PATH_CONF)

		expect(Paths.CMT_PATH_CONF).toBeMissingFile()
		expect(Paths.BM_PATH_ACCS).toBeMissingFile()
		expect(Paths.CMT_PATH_CONF_NODE_KEY).toBeMissingFile()
		expect(Paths.CMT_PATH_CONF_GENESIS).toBeMissingFile()
		expect(`${Paths.CMT_PATH_CONF}/priv_validator_key`).toBeMissingFile()
		expect(Paths.CMT_PATH_CONF_PRIV_VALIDATOR_KEY).toBeMissingFile()
		...
		expect(Paths.CMT_PATH_CONF).toHaveFile()
		expect(Paths.BM_PATH_ACCS).toHaveFile()
		expect(Paths.CMT_PATH_CONF_NODE_KEY).toHaveFile()
		expect(Paths.CMT_PATH_CONF_GENESIS).toHaveFile()
		expect(Paths.CMT_PATH_CONF_PRIV_VALIDATOR_KEY).toHaveFile()
