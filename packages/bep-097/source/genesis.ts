...
/**
 * This function generates configuration for a single node in the `~/.cometbft` directory.
 *
 * @remarks
	...
	for (let index = 0; index < validators.length; index++) {
		...
		if (validators[index] !== undefined) {
			persistValidatorKey({
				...
				path: options.entities.validators === 1 ? basePath : `${basePath}/${moniker}/.cometbft`,
				type: options.genesis.keyPairType,
			})
		}
		...
		// Write Genesis...
		persistGenesis({ ...options, path: `${path}/.cometbft`, state })

		// Modify Tendermint...
		modifyTendermintConfig(`${path}/.cometbft`, moniker, persistentPeers)
