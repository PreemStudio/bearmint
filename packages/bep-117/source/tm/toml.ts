...
export function readConfig(): Record<string, any> {
	return parse(file.read(Paths.CMT_PATH_CONF_CONFIG))
}

export function writeConfig(config: JsonMap) {
	file.write(Paths.CMT_PATH_CONF_CONFIG, stringify(config))
}
...
export function modifyConfig(
	...
	let data = file.read(Paths.CMT_PATH_CONF_CONFIG)
	...
	file.write(Paths.CMT_PATH_CONF_CONFIG, data)
}
